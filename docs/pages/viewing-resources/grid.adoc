...
The resources in a {doctitle} can be sorted in ascending or descending manner in various ways:
...
| Small | 3 rows
| Medium | 5 rows
| Large | 8 rows2.0.0-milestone.1.1
| Extra large | 12 rows
|===
...
The {doctitle} can be configured to only show autocraftable items or only non-autocraftable items.
